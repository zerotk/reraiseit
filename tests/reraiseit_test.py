...
import locale  # noqa - Used by code inside a string.
import pytest
import six
import traceback
...
if six.PY2:
    parametrized_exceptions = pytest.mark.parametrize(
        ...
        [
            ...
            ExceptionTestConfiguration(
                IOError, "raise IOError('message')", 'message'),
            ExceptionTestConfiguration(
                SyntaxError,
                "in valid syntax",
                expected_inner_exception_message=(
            ...
            ExceptionTestConfiguration(
                UnicodeEncodeError,
                "u'£'.encode('ascii')",
                "'ascii' codec can't encode character u'\\xa3' "
            ...
            pytest.mark.xfail(
                ...
                reason="ExceptionToUnicode() assumes that the 'message' "
                "attribute is 'unicode'"
            )(
                ExceptionTestConfiguration(
                    MessageAttributeIsBytesException,
                    'raise MessageAttributeIsBytesException()',
                    ''
                )
            ),
            ExceptionTestConfiguration(
                OSError,
                "raise OSError(2, "
                "'£ message'.encode(locale.getpreferredencoding()))",
                '[Errno 2] £ message'),
            ExceptionTestConfiguration(
                OSError,
                "raise OSError(2, b'£ message')", '[Errno 2] £ message'),
            ExceptionTestConfiguration(
                IOError,
                "raise IOError(b'£ message')", '£ message',
                expected_traceback_message=(
                    'IOError: <unprintable IOError object>\n'
                )
            ),
            ExceptionTestConfiguration(
                Exception,
                "raise Exception(b'£ message')", '£ message'
            ),
        ], ids=[
            'ValueError',
            'KeyError',
    ...
    parametrized_exceptions = pytest.mark.parametrize(
        ...
        [
            ExceptionTestConfiguration(
                ValueError, "raise ValueError('message')", 'message'),
            ExceptionTestConfiguration(
                KeyError, "raise KeyError('message')", "'message'"),
            ExceptionTestConfiguration(
                OSError,
                "raise OSError(2, 'message')",
                '[Errno 2] message',
            ...
            ExceptionTestConfiguration(
                ...
                expected_traceback_message=(
                    ...
                    '    in valid syntax\n'
                    '     ^\n'
                    'zerotk.reraiseit._reraiseit.ReraisedSyntaxError: '
                    'invalid syntax\n'
                )
            ),
            ExceptionTestConfiguration(
            ...
            ExceptionTestConfiguration(
                UnicodeEncodeError,
                "'£'.encode('ascii')",
                "'ascii' codec can't encode character '\\xa3' in position 0: "
                "ordinal not in range(128)"
            ),
            ExceptionTestConfiguration(
                AttributeError, "raise AttributeError('message')", 'message'),

            # On Python 3 (Windows) raising OSError actually raises a
            # FileNotFoundError.
            ExceptionTestConfiguration(OSError, "raise OSError()"),
            ExceptionTestConfiguration(OSError, "raise OSError(1)", '1'),
        ], ids=[
...
def testReraiseKeepsTraceback(exception_configuration):
    ...
    for traceback_entry in e.traceback:
        try:
            ...
            # traceback_entry.path will be a string when the code was run using
            # "exec compile('code')". However this does not hold true if 'code'
            # contains a SyntaxError, which in that case 'traceback_entry.path'
            # will be a LocalPath
    ...
    assert (
        crash_entry.locals['code'] ==
        exception_configuration.string_statement
    )
